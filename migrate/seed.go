...
func seed() {
	...
	db.Create(&menus)
	users := []model.User{}
	users = append(users, model.User{ID: 1, Name: "田中太郎", Email: "tekitou@example.com", TrainingGroupID: 1})
	users = append(users, model.User{ID: 2, Name: "佐藤花子", Email: "tekitou@example.com", TrainingGroupID: 1})
	users = append(users, model.User{ID: 3, Name: "山田次郎", Email: "tekitou@example.com", TrainingGroupID: 2})
	db.Create(&users)
